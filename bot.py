...
async def filter_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if message.text and "дядя" in message.text.lower():
        ...
        # Убираем "дядя" из текста чтобы анализировать остаток
        clean = text_lower.replace("дядя", "").strip().strip("?!.,").strip()

        # ── 1. ВОПРОС "КТО" → выбираем случайного участника ──────────────────
        if "кто" in text_lower:
            ...
            # Извлекаем роль — всё после "кто" без лишних слов
            ...
            role_responses = [
                ...
                f"🔍 Дядя проверил всех и вынес вердикт: {mention} — <b>{role}</b> 😏",
                f"📋 Список изучен. {mention} — <b>{role}</b>. Поздравляю, заслужил(а)!",
                f"⚡ {mention} — <b>{role}</b>. Можешь гордиться 😎",
                f"🎭 Внимание! Сегодняшний(яя) <b>{role}</b> среди нас — {mention}!",
                f"🌟 Вселенная выбрала. {mention} — <b>{role}</b>. Спорить бесполезно 😌",
                f"🏆 По итогам тщательного анализа — {mention} забирает титул <b>{role}</b>!",
                f"👁 Дядя всё видит. Сегодня {mention} — <b>{role}</b>. Прячься не прячься 😈",
            ]
        ...
        person_q_words = [
            "что делает", "что сделал", "что делала", "что сделала",
            "чё делает", "чё делал", "чё делала", "чем занимается",
            "чём занимается", "что творит", "что там", "где сейчас",
            "где он", "где она", "куда пошёл", "куда пошла", "куда делся",
            "откуда", "как дела", "как он", "как она", "кто такой", "кто такая",
            "что с ним", "что с ней", "что происходит", "когда придёт",
            "когда придет", "почему не", "зачем он", "зачем она",
        ]
        is_person_question = any(pq in clean for pq in person_q_words)

        # Дополнительно: "дядя [слово] где/что/как/куда" — простые слова
        if not is_person_question:
            words_after = clean.split()
            if (len(words_after) >= 2 and
                    words_after[-1] in ["где", "куда", "зачем", "почему", "когда"]):
        ...
        if is_person_question:
            ...
            if name:
                deflect_responses = [
                    ...
                    f"Иди у {name}а сам спроси, зачем ко мне пристал 😤",
                ...
                deflect_responses = [
                    "Я что, телефонная книжка? 📱 Иди сам спроси!",
                    "Откуда я знаю?! Я дядя, а не Яндекс 🗺",
                    "Понятия не имею, и знать не хочу 😌",
                    "Иди сам спроси, чё ко мне пристал 😤",
                    "Я что, слежу за всеми? Ты серьёзно? 👀",
                    "Ошибся адресом, дружок 🙅",
                    "Не знаю, не видел, не слышал 🙈🙉",
                    "Спроси у него/неё сам(а). Я здесь при чём? 😒",
                    "Дядя занят, личные вопросы — не по адресу 📵",
                    "Гугли. Я не поисковик 🔍",
                ]
            await message.reply_text(random.choice(deflect_responses))
            return

        # ── 3. ТЕМАТИЧЕСКИЕ ОТВЕТЫ по контексту ──────────────────────────────
        themes = [
            (["спать", "спи", "сон", "ночь", "ложись", "поздно", "баиньки"], [
                "Сам иди спать, дядя ещё не нагулялся 🌙",
                "Спать?! Да я только разогрелся 🔥",
                "Поздно? Дядя живёт по своему часовому поясу 😎",
                "Уже лежу... нет, передумал 🛌",
                "Дядя спит только когда захочет. А хочет — никогда 💪",
            ]),
            (["петь", "пой", "пение", "песню", "песня", "музыка"], [
                "Петь? Дядя берёт за концерт 💰",
                "Голос есть, но только за донат 🎤",
                "Спою, но только рэп. Другого не держим 🎵",
                "Репертуар закончился, приходи завтра 🎶",
                "Микрофон подними сначала, потом поговорим 🎙",
            ]),
            (["танцевать", "танцуй", "потанцуй", "танец", "станцуй"], [
                "Дядя танцует только в пятницу 🕺",
                "Танцы? Колено болит, извини 🤕",
                "Флексить умею, но не сегодня 😅",
                "Только если музыка нормальная. А у тебя нормальная? 🤔",
            ]),
            (["иди", "уходи", "вали", "убирайся", "уйди", "пошёл"], [
                "Сам иди 😴",
                "Не дождёшься 😏",
            ...
            (["замолчи", "заткнись", "помолчи", "тихо", "молчи"], [
                "Нет 😐",
                "Не замолчу, и что? 😌",
                "Дядя молчит только когда спит. А я не сплю 😏",
                "Слова не кончатся, не надейся 🗣",
            ]),
            (["помоги", "помогай", "помощь", "выручи"], [
                "Помочь? С удовольствием... нет 😂",
                "Техподдержка не работает, попробуй позже 📞",
                "Дядя занят, перезвони после праздников 🗓",
                "Чем могу помочь? Ничем. Но спасибо за вопрос 😇",
            ]),
            (["работать", "работай", "работа", "займись делом"], [
                "Дядя уже работает — следит за вами всеми 👁",
                "Работаю 24/7 пока вы спите 💼",
                "Это и есть моя работа 😏",
            ]),
            (["анекдот", "расскажи", "история", "шутка", "пошути"], [
                "Анекдот: ты попросил дядю рассказать анекдот. Смешно, правда? 😂",
                "История длинная, читай сам 📚",
                "Дядя — не клоун. Хотя... иногда 🤡",
                "Шутка дня: этот чат 😂",
            ]),
            (["который час", "сколько время", "время", "час"], [
                f"Время {datetime.now(timezone.utc).strftime('%H:%M')} по UTC. Считай сам по своему часовому поясу 🕐",
                "Время идти работать 😏",
                "Время — деньги. А деньги — у дяди 💰",
            ]),
        ]

        responded = False
        for keywords, responses in themes:
        ...
        if not responded:
            # Если контекст не распознан — выбираем случайного участника как шлюху дня
            members = [u for uid, u in group_members.get(chat_id, {}).items() if uid != user.id]
            if not members:
                try:
                    chat_admins = await context.bot.get_chat_administrators(chat_id)
