...
async def filter_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if message.text and "дядя" in message.text.lower():
        ...
        orig = message.text
        clean = text_lower.replace("дядя", "").strip().strip("?!., ").strip()

        # Вспомогательная функция: найти имя в сообщении (заглавное или строчное)
        _skip = {
            "дядя", "кто", "такой", "такая", "что", "чё", "чо", "где", "как",
            "куда", "откуда", "когда", "зачем", "почему", "кого", "кому",
            "чем", "кем", "чей", "чья", "он", "она", "они", "его", "её",
            "там", "тут", "это", "не", "ну", "да", "нет", "же", "а", "и",
            "или", "но", "в", "на", "за", "по", "из", "от", "до", "у",
            "делает", "делал", "делала", "пошёл", "пошла", "будет",
        }
        def _extract_name() -> str | None:
            # Сначала ищем слово с заглавной буквы
            caps = re.findall(r'[А-ЯЁA-Z][а-яёa-z]{1,}', orig)
            for w in caps:
                if w.lower() not in _skip:
                    return w
            # Если нет заглавных — ищем среди строчных
            words = re.sub(r'[^а-яёa-z\s]', ' ', text_lower).split()
            for w in reversed(words):  # берём последнее подходящее (чаще всего имя в конце)
                if w not in _skip and len(w) > 2:
                    return w.capitalize()
            return None

        # ── 1. КТО ТАКОЙ / КТО ТАКАЯ [ИМЯ] — гендерный ответ ────────────────
        if "такой" in text_lower or "такая" in text_lower:
            name = _extract_name()
            is_female = "такая" in text_lower

            if name and is_female:
                responses = [
                    f"О, {name}! 👀 Это та самая девочка, которая делает вид что тихоня, а сама ещё та штучка 😏",
                    f"{name}? Знаю-знаю... глаза невинные, а в голове — ого-го 🔥",
                    f"Та самая {name} которая говорит 'я устала', но первой не уходит с вечеринки 😈",
                    f"{name} — это легенда. Днём — ангел, ночью — вопрос открытый 😇🔥",
                    f"Аааа, {name}! Та что улыбается всем, а думает только о своём 😌💋",
                    f"{name}? Внешне — само спокойствие. Внутри — хаос и огонь 🌪🔥",
                    f"Да ладно, все знают кто такая {name}. Но вслух не говорим 😏🤐",
                    f"{name} — человек с секретом. И этот секрет явно интересный 👀",
                    f"Хм, {name}... Скажу так: за тихим омутом черти водятся 😈",
                    f"{name} та ещё история. Снаружи — всё прилично, а внутри — библиотека грехов 📚😈",
                ]
            elif name and not is_female:
                responses = [
                    f"{name}? Обычный мужик. Но с претензиями 😂",
                    f"О, {name}! Тот самый который думает что он альфа, но мы-то знаем 😏",
                    f"{name} — это человек у которого всегда 'всё нормально', но ничего не нормально 🙃",
                    f"Знаю {name}. Типичный: днём герой, ночью в телефоне 📱😂",
                    f"{name}? Хороший парень. Но это пока не знаешь поближе 😈",
                    f"Ааа, {name}! Тот что строит из себя серьёзного, но первый смеётся над глупостями 😂",
                    f"{name} — легенда чата. Хотя сам об этом не знает 🤷",
                    f"Скажу честно про {name}: внешне — загадка, внутри — ещё большая загадка 🤔",
                    f"{name} тот самый тип который молчит-молчит, а потом выдаёт такое... 😂🔥",
                    f"О {name} можно много говорить. Но лучше просто наблюдать 👁️",
                ]
            else:
                responses = [
                    "Кто такой? Загадочная личность 🤔",
                    "Не знаю, но звучит интригующе 😏",
                ]
            await message.reply_text(random.choice(responses))
            return

        # ── 2. КТО [РОЛЬ] — без имени → выбираем случайного участника ────────
        if "кто" in text_lower:
            members = [u for uid, u in group_members.get(chat_id, {}).items() if uid != user.id]
            if not members:
                try:
                    chat_admins = await context.bot.get_chat_administrators(chat_id)
                    members = [m.user for m in chat_admins if not m.user.is_bot and m.user.id != user.id]
                except Exception:
                    members = []
            if not members:
            ...
            for filler in ["у нас", "тут", "здесь", "из нас", "среди нас", "в чате", "самый", "самая", "дядя", "?"]:
                after_kto = after_kto.replace(filler, " ")
            role = re.sub(r'\s+', ' ', after_kto).strip().strip("?!.,").strip()
            if len(role) < 2 or len(role) > 60:
                role = "шлюха дня"
            role_responses = [
                f"🎯 Без сомнений — сегодня <b>{role}</b> это {mention}!",
                f"🔍 Дядя проверил всех: {mention} — <b>{role}</b> 😏",
                f"📋 Вердикт вынесен. {mention} — <b>{role}</b>. Поздравляю!",
                f"⚡ {mention} — <b>{role}</b>. Можешь гордиться 😎",
                f"🎭 Внимание! Сегодняшний(яя) <b>{role}</b> — {mention}!",
                f"🌟 Вселенная выбрала. {mention} — <b>{role}</b>. Спорить бесполезно 😌",
                f"🏆 Итог анализа: {mention} забирает титул <b>{role}</b>!",
                f"👁 Дядя всё видит. {mention} — <b>{role}</b>. Прячься не прячься 😈",
            ]
            await message.reply_html(random.choice(role_responses))
            return

        # ── 3. ВОПРОСЫ О ЧЕЛОВЕКЕ (где/что делает/куда) → отшиваем ──────────
        q_words = {"что", "чё", "чо", "где", "как", "куда", "откуда", "когда",
                   "зачем", "почему", "кого", "кому", "чем", "кем", "чей", "чья"}
        clean_words_list = re.sub(r'[^а-яёa-z\s]', ' ', clean).split()
        has_q = any(w in q_words for w in clean_words_list) or "?" in orig
        name = _extract_name()
        if has_q and name:
            deflect = [
                f"Я что, нянька {name}а? 😤 Иди сам(а) спроси!",
                f"Откуда мне знать что там {name} делает 🤷 Я дядя, а не шпион",
                f"Слежу за {name}ом? Ты серьёзно? 😂 Иди сам разберись",
                f"Вот прямо сейчас позвоню {name}у и спрошу... НЕТ 📵",
                f"Я что, личный секретарь {name}а? 😒",
                f"{name}? Понятия не имею, и знать не хочу 😌",
        ...
        # ── 4. ДЕЙСТВИЯ — дерзкие шуточные ответы ────────────────────────────
        action_themes = [
            (
                ["спать", "спи", "ложись", "сон", "баиньки", "поздно", "ночь"],
                [
                    ...
                    "Уже лежу... нет, передумал. Чего спросил? 🛌",
                ]
            ),
            (
                ["гулять", "гуляй", "погуляй", "прогуляйся"],
                [
                    ...
                    "Зачем гулять, если можно сидеть здесь и троллить тебя? 😈",
                ]
            ),
            (
                ...
                [
                    ...
                    "Микрофон в руке, но петь тебе — это слишком большая честь 😏",
                    "Пою только для тех кто заслуживает. Ты — нет 😂",
                ]
            ),
            (
                ...
                [
                    ...
                    "Танцы? Ты сначала сам попробуй 😂",
                    "Флексить умею. Но не перед тобой 😏",
                    "Только если сам зажжёшь — тогда посмотрим 🔥",
                    "Я танцую когда хочу. А сейчас не хочу 🙅",
                ]
            ),
            (
            ...
            (
                ...
                [
                    ...
                    "Моя работа — отвечать тебе. Вот и отвечаю. Доволен? 🙂",
                ]
            ),
            (
        ...
        mention = f'<a href="tg://user?id={chosen.id}">{chosen.first_name}</a>'
        default_responses = [
            f"🌹 Сегодняшняя звезда нашего чата — {mention}. Аплодисменты!",
            f"✨ Вселенная выбрала. Судьба указала. Сегодня это {mention} — шлюха дня! 💫",
            f"🎭 Титул шлюхи дня торжественно вручается {mention}!",
            f"🤖 Сложнейший анализ завершён: {mention} — шлюха дня.",
            f"📊 Статистика не врёт. Сегодня это {mention} 😏",
            f"🍑 Сегодня самая горячая штучка нашего чата — {mention} 😈",
            f"🔥 {mention}, сегодня твой звёздный день! Шлюха дня — это ты 😘",
            f"😏 Дядя выбрал {mention}. Сегодня ты — главная шлюха чата. Гордись!",
        ]
